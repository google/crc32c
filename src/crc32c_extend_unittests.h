...
TEST(TESTED_EXTEND_CASE_NAME, StandardResults) {
  ...
  EXPECT_EQ(
      static_cast<std::uint32_t>(0x62a8ab43),
      TESTED_EXTEND(0, buf, sizeof(buf)));
  ...
  EXPECT_EQ(
      static_cast<std::uint32_t>(0x46dd794e),
      TESTED_EXTEND(0, buf, sizeof(buf)));
  ...
  EXPECT_EQ(
      static_cast<std::uint32_t>(0x113fdb5c),
      TESTED_EXTEND(0, buf, sizeof(buf)));
  ...
  EXPECT_EQ(
      static_cast<std::uint32_t>(0xd9963a56),
...
TEST(TESTED_EXTEND_CASE_NAME, HelloWorld) {
  ...
  EXPECT_EQ(
      TESTED_EXTEND(0, hello_space_world, 11),
