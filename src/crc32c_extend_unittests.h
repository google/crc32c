...
TEST(TESTED_EXTEND_CASE_NAME, StandardResults) {
  ...
  EXPECT_EQ(static_cast<std::uint32_t>(0x8a9136aa),
            TESTED_EXTEND(0, buf, sizeof(buf)));

  std::memset(buf, 0xff, sizeof(buf));
  EXPECT_EQ(static_cast<std::uint32_t>(0x62a8ab43),
            TESTED_EXTEND(0, buf, sizeof(buf)));

  for (int i = 0; i < 32; ++i)
    buf[i] = static_cast<std::uint8_t>(i);
  EXPECT_EQ(static_cast<std::uint32_t>(0x46dd794e),
            TESTED_EXTEND(0, buf, sizeof(buf)));

  for (int i = 0; i < 32; ++i)
    buf[i] = static_cast<std::uint8_t>(31 - i);
  EXPECT_EQ(static_cast<std::uint32_t>(0x113fdb5c),
            TESTED_EXTEND(0, buf, sizeof(buf)));
...
TEST(TESTED_EXTEND_CASE_NAME, BufferSlicing) {
  ...
  for (std::size_t i = 0; i < 48; ++i) {
    for (std::size_t j = i + 1; j <= 48; ++j) {
      ...
      EXPECT_EQ(static_cast<std::uint32_t>(0xd9963a56), crc);
    }
  }
}
